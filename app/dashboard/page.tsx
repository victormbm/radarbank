...
const BANK_DETAIL_CACHE_TTL_MS = 5 * 60 * 1000;
...
function statusBadgeClass(status?: IngestStatus["status"]) {
  if (status === "fresh") return "bg-emerald-100 text-emerald-700 border-emerald-300";
  if (status === "current") return "bg-blue-100 text-blue-700 border-blue-300";
  if (status === "stale") return "bg-amber-100 text-amber-700 border-amber-300";
  if (status === "outdated") return "bg-rose-100 text-rose-700 border-rose-300";
  return "bg-slate-100 text-slate-700 border-slate-300";
}

function statusBadgeLabel(status?: IngestStatus["status"]) {
  if (status === "fresh") return "Atualizado hoje";
  if (status === "current") return "Atualizado";
  if (status === "stale") return "Base antiga";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const cached = bankDetailCacheRef.current.get(cacheKey);
    const isCacheValid = cached && (Date.now() - cached.fetchedAt) < BANK_DETAIL_CACHE_TTL_MS;

    if (isCacheValid) {
      setSelectedBankDetail(cached.data);
      setIsLoadingDetail(false);
      return;
    }
    ...
    const loadBankDetail = async () => {
      ...
      try {
        ...
        setSelectedBankDetail(null);
        const response = await fetch(`/api/banks/${cacheKey}`, { signal: controller.signal });
